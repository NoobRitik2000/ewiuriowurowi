...
export const loginUser = async (req: Request, res: Response) => {
    ...
    // Access the dataValues
        ...
        const token = jwt.sign({ id: user.user_id, role: user.role }, 'jwt_secret_key', { expiresIn: '24h' });
...
// Function to add a new librarian by admin
// Function to add a new librarian by admin
export const addLibrarian = async (req: Request, res: Response) => {
    const { name, email, password, role } = req.body;
   console.log(name, email, password, role,"fjkdshfjksdhfkjda");
   
    try {
        // Check if a user with the same email already exists
        const existingUser = await User.findOne({ where: { email } });
        console.log(existingUser,"jskdjkdfj");
        ...
        // Create a new librarian with the role set to 'librarian' by default
        const librarian = await User.create({
            name,
            email,
            password: hashedPassword,
            role: role || 'librarian', // Use provided role or default to 'librarian'
        });

        res.status(201).json({ message: 'Librarian added successfully.', librarian });
    } catch (error) {
        console.error('Error adding librarian:', error);
        res.status(500).json({ error: 'Failed to add librarian. Please check your input.' });
    }
};
